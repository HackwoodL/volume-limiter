...
volume-limit get                    # 打印当前上限、音量和设备（简略）
...
volume-limit headphone-only on      # 仅限制耳机类输出设备
volume-limit headphone-only off     # 限制所有输出设备
volume-limit headphone-only status  # 打印「仅限制耳机」是否开启
...
因为本人是个穷鬼，没钱买 Apple 开发者签名（Apple Developer Program），所以发布版本只做 ad-hoc 签名，不走 Developer ID，也不做 notarization。因此首次打开时 macOS 可能需要你右键打开、执行 `xattr -cr`，或在系统设置中选择“仍要打开”。
