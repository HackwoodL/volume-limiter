...
- 分设备上限：每个输出设备记住自己的上限，未单独设置的设备使用全局默认值。
- 支持音量被压回时发送 macOS 通知。
...
```bash
volume-limit set <0-100>        # 设置当前正在使用的设备的上限
volume-limit reset              # 当前设备恢复使用默认上限
volume-limit default <0-100>    # 设置其他设备的默认上限
volume-limit device list        # 列出各设备的上限
volume-limit get
volume-limit on
volume-limit off
volume-limit status
